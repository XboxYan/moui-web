...
const ResizeW = (props) => {
    ...
    const resize = (target, event, { left, top, width, height, _X, _Y, l, t, _w, _h }, type) => {
        ...
        target.dataset.resW = parseInt(w,10);
        target.dataset.resH = parseInt(h,10);
        target.dataset.posX = parseInt(x,10);
        target.dataset.posY = parseInt(y,10);
        res = { target, w, h, x, y };
    }
...
class View extends PureComponent {
    ...
    darg = (target, event, { _X, _Y, _left, _top }) => {
        ...
        const centX = width*.5 - offsetWidth*.5;
        const centY = height*.5 - offsetHeight*.5;
        if (overObj) {
            util.addClass(overObj, 'in');
            if (this.overObj && this.overObj !== overObj) {
                util.removeClass(this.overObj, 'in');
            }
        ...
        if(overObj.dataset.allowchild==='true'){
    ...
    dargEnd = (event) => {
        ...
        if (overTarget&&overTarget.dataset.allowchild==='true') {
            util.removeClass(overTarget, 'in');
    ...
    render() {
        const { className, resizeEnd, x, y, w, h,allowchild } = this.props;
        const { editable } = this.state;
